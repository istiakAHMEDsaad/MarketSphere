...
const MyPostedJobs = () => {
  ...
  const fetchDataByEmail = async () => {
    const { data } = await axiosInstance.get(`/jobs/user-jobs/${user?.email}`);
    return data.jobs;
  };
  ...
  const {
    ...
    queryFn: fetchDataByEmail,
    enabled: !!user?.email, //run if user exist
  });

  const deleteItem = useMutation({
    mutationFn: async (id) => {
      const { data } = await axiosInstance.delete(`/jobs/job/${id}`);
      return data;
    },
                  ...
                  <tbody className='bg-white divide-y divide-gray-200 '>
                    {jobs?.map((item) => (
                      <tr key={item._id}>
                        ...
                        <td className='px-4 py-4 text-sm text-gray-500  whitespace-nowrap'>
                          {item.deadline && format(item.deadline, 'P')}
                        </td>
